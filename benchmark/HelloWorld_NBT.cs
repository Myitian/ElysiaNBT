...
[MemoryDiagnoser]
[MarkdownExporter]
public class HelloWorld_NBT
{
    static byte[] Data;
    [GlobalSetup]
    public void Setup()
    {
        using Stream s = Program.Entries.First(e => e.Name == "hello_world.nbt").Open();
    ...
    public void Benchmark_ElysiaNBT_ToObject()
    {
        ...
        ElysiaNBT.BinaryNbtOptions options = ElysiaNBT.BinaryNbtOptions.JavaEdition;
        options.GenericOptions.HasRootName = true;
        HelloWorld result = ElysiaNBT.Serialization.NbtSerializer.DeserializeBinary<HelloWorld>(stream, options);
    }
    ...
    {
        using MemoryStream stream = new(Data);
        Konves.Nbt.Serialization.NbtSerializer serializer = new(typeof(HelloWorld));
    ...
    public void Benchmark_NbtLib_Debug_ToObject()
    {
        ...
        HelloWorld result = deserializer.DeserializeObject<HelloWorld>(stream);
    }
